...
public:
	SRWebcamVideoStreamMF() :
		context(SRWebcamMFContext::getContext()) {}
...
#	pragma warning(pop)
	...
	bool setupWith(int id, int framerate, int w, int h) {
		...
		if(!SUCCEEDED(videoReader->SetStreamSelection((DWORD)MF_SOURCE_READER_ALL_STREAMS, false)) || !SUCCEEDED(videoReader->SetStreamSelection((DWORD)bestStream, true)) || !SUCCEEDED(videoReader->SetCurrentMediaType((DWORD)bestStream, NULL, typeOut))) {
			videoReader->Release();
			videoReader = NULL;
			ppDevices[_id]->Release();
			CoTaskMemFree(ppDevices);
